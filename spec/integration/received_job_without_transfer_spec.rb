...
describe 'Received Job Without Transfer' do

  context 'when payment collection is allowed ' do

    context 'when allowing to transfer' do
      ...
      it 'only the provide is allowed to collect before accepting the job' do
      ...
      context 'when accepted' do
        ...
        it 'the  available payment status events should be: collect and provider_collected' do
        ...
        context 'when started the work' do
          ...
          context 'when completed the job' do
            ...
            it 'the available payment events should be: invoice, invoiced by prov, collect, provider_collectd' do
              expect(job.billing_status_events).to eq [:invoice, :provider_invoiced, :provider_collected, :collect]
            ...
            context 'when I invoice' do
              ...
              context 'when I collect' do
                ...
                context 'when collecting cash' do
                  before do
                    job.update_attributes(billing_status_event: 'collect', payment_amount: '10000', payment_type: 'cash', collector: org)
                    job.reload
                    job.payment_amount = nil # to simulate a new user request
                  end
  ...
  context 'when payment collection is NOT allowed' do
    context 'when allowing to transfer' do
      include_context 'job transferred from a local provider' do
        let(:collection_allowed?) { false }
        let(:transfer_allowed?) { true }
      end
      pending
    end

    context 'when not allowing to transfer' do
      include_context 'job transferred from a local provider' do
        let(:collection_allowed?) { false }
        let(:transfer_allowed?) { false }
      end
      pending
    end
  end
