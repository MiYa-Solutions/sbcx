...
describe DepositEntryConfirmedEvent do

  include_context 'entry event mocks' do
    let(:event) { DepositEntryConfirmedEvent.new(eventable: ticket, entry_id: entry.id, triggering_event: mock_model(DepositEntryConfirmEvent)) }
  end
  ...
  context 'when created' do
    ...
    it 'should change the entry status to disputed' do
      orig_entry.should_receive(:confirmed!)
      event.save!
    end
  ...
  context 'validation' do
    ...
    it 'should have a reference id of 300014' do
      expect(event.reference_id).to be 300014
    end

    it 'should have a description taken from I18n' do
      expect(event.description).to eq I18n.t('entry_confirmed_event.description')
    end

    it 'should have a description taken from I18n' do
      expect(event.name).to eq I18n.t('entry_confirmed_event.name')
    end
  end
end