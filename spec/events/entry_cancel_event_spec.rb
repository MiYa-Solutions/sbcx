...
describe EntryCancelEvent do

  include_context 'entry event mocks' do
    let(:event) { EntryCancelEvent.new(eventable: ticket, entry_id: orig_entry.id) }
  end
  ...
  context 'when created' do
    ...
    before do
      ...
      EntryCanceledEvent.stub(:new).with(kind_of(Hash)).and_return(double('EntryCanceledEvent'))
      Event.stub_chain(:where, :where).and_return([adj_event])
    end

    it 'should create EntryCancledEvent when processing the event' do
      EntryCanceledEvent.should_receive(:new)
      event.save!
    end
  ...
  context 'validation' do
    ...
    it 'should have a reference id of 300015' do
      expect(event.reference_id).to be 300015
    end

    it 'should have a description taken from I18n' do
      expect(event.description).to eq I18n.t('entry_cancel_event.description')
    end

    it 'should have a description taken from I18n' do
      expect(event.name).to eq I18n.t('entry_cancel_event.name')
    end
  end
end