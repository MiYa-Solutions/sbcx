...
describe ScStartedNotification do
  let(:service_call) { FactoryGirl.create(:my_service_call, subcontractor: FactoryGirl.create(:member).becomes(Subcontractor)) }


  it "is created once the service call is transferred and started to the subcontractor" do
    ...
    }.to change { ScStartedNotification.count }.by (1)
  ...
  describe " default content and subject" do
    let(:notification) {
      ...
      service_call.notifications.where(type: ScStartedNotification).last
