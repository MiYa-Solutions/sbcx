shared_context 'entry event mocks' do
  ...
  let(:ticket) { mock_model(Ticket, id: 1, name: 'Test Ticket', events: [], subcontractor: org2, provider: org, organization: org, provider_agreement: agr, subcon_agreement: agr) }
  let(:ticket2) { mock_model(Ticket, id: 2, name: 'Test Ticket2', events: [], subcontractor: org2, provider: org, organization: org, provider_agreement: agr, subcon_agreement: agr) }
  let(:acc) { mock_model(Account, id: 1, name: 'Test Acc', organization: org, accountable: org2, events: [], entries: []) }
