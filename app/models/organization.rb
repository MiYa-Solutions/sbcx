...
class Organization < ActiveRecord::Base
  ...
  scope :the_affiliates, ->(org) { joins("INNER JOIN agreements on ((agreements.organization_id = organizations.id OR agreements.counterparty_id = organizations.id) AND agreements.counterparty_type = 'Organization') WHERE (agreements.organization_id = #{org.id} OR agreements.counterparty_id = #{org.id}) AND organizations.id != #{org.id}") }
