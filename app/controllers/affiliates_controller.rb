class AffiliatesController < ApplicationController
  ...
  def index
    if params[:search].nil?
      @affiliates = Affiliate.my_affiliates(current_user.organization.id).order("organizations.id DESC").paginate(page: params[:page], per_page: 5)
    else
  ...
  def show
    ...
    @account    = Account.where("organization_id = ? AND accountable_id = ? AND accountable_type = 'Organization'", current_user.organization.id, @affiliate.id).first
  end
